...
pub fn sketch_sequences_from_bytes(
    ...
    let mut kmer_map: HashMap<u64, u32, _> = HashMap::default();
    let mut kmer_to_pair_table = FxHashSet::default();
    let mut num_dup_removed = 0;
    let mut mean_read_length = 0.0_f64;
...
pub fn sketch_pair_sequences_from_bytes(
    ...
    let mut kmer_counts: HashMap<u64, u32, _> = HashMap::default();
    let mut kmer_pair_set: FxHashSet<(u64, [Marker; 2])> = FxHashSet::default();
    let mut num_dup_removed = 0;
    let mut mean_read_length = 0.0_f64;
    let mut counter = 0.0_f64;
    ...
    loop {
        ...
        extract_markers(&rec1.seq(), &mut temp1, c, k);
        extract_markers(&rec2.seq(), &mut temp2, c, k);
        let kmer_pair = pair_kmer(&rec1.seq(), &rec2.seq());

        counter += 1.0;
        mean_read_length += ((rec1.seq().len() as f64) - mean_read_length) / counter;
